# Migrated from python_app/broker/finvasia.py — DO NOT EDIT HERE
import logging
import requests
import pyotp
    ...
    def place_order(self, order_details: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('status') or data.get('success'):
                    return str(data.get('order_id', data.get('data', {}).get('nsefo', {}).get('order_id', '')))
                else:
                    self.logger.warning(f"Finvasia Order Rejected: {data.get('message')}")
            return ""
        except Exception as e:
